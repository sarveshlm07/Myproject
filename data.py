...
def _require_kite():
    if kite is None:
        raise RuntimeError(
            "Missing Zerodha credentials. Set NIFTY_API_KEY and NIFTY_ACCESS_TOKEN environment variables."
        )
...
def _current_bucket(now, seconds):
    ...
    return pd.Timestamp(anchor) + pd.Timedelta(seconds=(elapsed // seconds) * seconds)
...
def _ensure_single_running_candle(df, interval, now):
    """Single source of truth for the running candle.

    Historical rows are never blindly overwritten. We identify the exact
    wall-clock bucket first and only create/update that bucket.
    """
    if df is None or df.empty:
        return df

    _, seconds = _interval_info(interval)
    bucket = _align_timestamp(_current_bucket(now, seconds), df.index)
    ...
    if bucket in x.index:
        ...
        if live_price is not None:
            x.loc[bucket, "High"] = max(float(row["High"]), live_price)
            x.loc[bucket, "Low"] = min(float(row["Low"]), live_price)
...
def get_nifty_data(interval="5m"):
    ...
    # Strictly current trading day only. Full-session structure remains clean.
    try:
        today = now.date()
    ...
    # Only data.py owns running-candle construction/update.
    if now <= market_close:
        df = _ensure_single_running_candle(df, interval, now)
    return df
